    ...
    ui(new Ui::ClientForm)
{
    ...
    connect(ui->tableView, SIGNAL(cellClicked(int,int)),this,
    ...
    //queryModel.setQuery("CALL UPDATE_ADDRESS(1000)");
    //queryModel.setQuery("SELECT * FROM SHOPPING s, CLIENT c, PRODUCT p WHERE s.client_id = c.client_id and s.product_id = p.product_id");
    clientQueryModel->setQuery("SELECT * FROM CLIENT");
//    queryModel.setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
//    queryModel.setHeaderData(1, Qt::Horizontal, QObject::tr("Client Name"));
...
//    ui->tableView->setWindowTitle(QObject::tr("Client Table"));
//    ui->tableView->show( );
...
//    int tableRowCount = ui->tableView->rowCount();
//    // 반복횟수를 위해 변수에 행의 개수를 저장
//    for (int i = 0; i < tableRowCount; i++) {
//        // 스트림 연산자를 통해 쉼표로 데이터를 구분해서 저장, 마지막엔 줄바꿈 문자 삽입
//        out << ui->tableView->item(i, 0)->text() << ","
//            << ui->tableView->item(i, 1)->text() << ","
//            << ui->tableView->item(i, 2)->text() << ","
//            << ui->tableView->item(i, 3)->text() << "\n";
//    }
...
{
//    int lastColumn = ui->tableView->rowCount() - 1;   // 마지막 행의 값을 가져옴

//    if( lastColumn < 0 )   // 데이터가 없을 경우 1000번부터 ID를 부여
//        return 1000;

//    int creatId = ui->tableView->item(lastColumn, 0)->text().toInt() + 1;
